...
```bash
docker run -t -i nu11secur1ty/snort-2.9.16 /bin/bash
```
- - - Dirrect to `/opt/`

```
docker run -it --rm --net=host nu11secur1ty/snort-2.9.16 /bin/bash
```
***IMPORTANT:***
...
```
docker run -it --rm --net=host --cap-add=NET_ADMIN nu11secur1ty/snort-2.9.16 /bin/bash
```
