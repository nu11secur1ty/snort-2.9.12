...
RUN yum -y install ${BASE_URL}/snort/daq-${DAQ_VERSION}-1.centos7.x86_64.rpm \
                      ${BASE_URL}/snort/snort-${SNORT_VERSION}-1.centos7.x86_64.rpm 
RUN mkdir -p /etc/snort/rules 
RUN curl -sSL ${BASE_URL}/community/community-rules.tar.gz 
RUN tar xz --strip 1 -C /etc/snort/rules/ community-rules/community.rules \
...
RUN ln -s /usr/lib64/libdnet.so.1 /usr/local/lib/libdnet.1 
RUN yum clean all
